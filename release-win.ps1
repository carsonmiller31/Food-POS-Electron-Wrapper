...
Write-Log ("Done. Release URL: https://github.com/{0}/{1}/releases/tag/{2}" -f (git config --get remote.origin.url -replace '^.*github.com[:/]', '' -replace '\.git$', '' -split '/')[0], (git config --get remote.origin.url -replace '^.*github.com[:/]', '' -replace '\.git$', '' -split '/')[1], $tag)
